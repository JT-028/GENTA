...
function initPage() {
            ...
            $('.defaultDataTable').each(function(){
                ...
                try {
                    if ($.fn.DataTable.isDataTable($tbl)) {
                        ...
                        try { tblApi.columns().adjust(); } catch(e) { /* noop */ }
                        try { if (tblApi.responsive) tblApi.responsive.recalc(); } catch(e) { /* noop */ }
                    } else {
                    ...
                    console.warn('DataTable init/adjust failed for .defaultDataTable', e);
                }
            });
        }
    } catch (e) {
    ...
    try {
        if ($ && $.fn && typeof $.fn.inputmask === 'function') {
            ...
            $('[name="lrn"]').each(function(){ try { $(this).inputmask('999999999999', {placeholder: ''}); } catch(e) { /* noop */ } });
            $('[name="student_code"]').each(function(){ try { $(this).inputmask('999999999999', {placeholder: ''}); } catch(e) { /* noop */ } });
        }
    } catch (e) {
    ...
    try { updateActiveNav(); } catch(e) { /* ignore */ }

    // If the loaded page contains an updated profile image, sync it into the sidebar
    try {
        var $contentProfileImg = $('.content-wrapper').find('img[src*="/uploads/profile_images/"]').first();
        if ($contentProfileImg && $contentProfileImg.length) {
            ...
            if ($sidebarImg && $sidebarImg.length && $sidebarImg.attr('src') !== src) {
                $sidebarImg.attr('src', src);
            }
        }
    } catch(e) { /* ignore */ }
    ...
    try {
        ...
        if ($profileForm && $profileForm.length) {
            ...
            // Preview file input
            $profileForm.find('input[type=file][name="profile_image"]').off('change.preview').on('change.preview', function(e){
                ...
                reader.onload = function(ev){
                    ...
                    if (!$img || $img.length === 0) {
                        $img = $('<img class="profile-preview" style="max-width:100px; margin-top:10px; display:block;">');
                        $profileForm.find('input[type=file][name="profile_image"]').after($img);
                    }
            ...
            $profileForm.off('submit').on('submit', function(e){
                ...
                fetch($(form).attr('action') || window.location.href, {
                    ...
                    console.log('[Profile Form] Response status:', resp.status);
                    return resp.text().then(function(text) {
                        console.log('[Profile Form] Raw response (first 500 chars):', text.substring(0, 500));
                        try {
                            return JSON.parse(text);
                        } catch(err) {
                            console.error('[Profile Form] JSON parse error:', err);
                    ...
                    if (data && data.success) {
                        ...
                        if (data.profile_image) {
                            var $sidebarImg = $('#sidebar .nav-profile-image img').first();
                            if ($sidebarImg && $sidebarImg.length) $sidebarImg.attr('src', data.profile_image);
                            // Also update any preview images
                            $profileForm.find('img.profile-preview').attr('src', data.profile_image);
                        }
                        if (data.full_name) {
                            $('#sidebar .nav-profile-text .font-weight-bold').text(data.full_name);
                        }
                        if (typeof Swal !== 'undefined') {
                            Swal.fire({ icon: 'success', toast: true, position: 'top-end', showConfirmButton: false, timer: 1800, title: data.message || 'Profile saved' });
                        } else {
                            alert(data.message || 'Profile saved');
                        }
                    } else {
                        if (typeof Swal !== 'undefined') {
                            Swal.fire({ icon: 'error', toast: true, position: 'top-end', showConfirmButton: false, timer: 3000, title: data && data.message ? data.message : 'Error saving profile' });
                        } else {
                            alert(data && data.message ? data.message : 'Error saving profile');
                        }
                    }
                }).catch(function(err){
                    console.error('Profile AJAX save failed', err);
                    if (typeof Swal !== 'undefined') {
                        Swal.fire({ icon: 'error', title: 'Network error', text: 'Unable to save profile. Please try again.' });
                    } else {
        ...
        if ($passwordForm && $passwordForm.length) {
            ...
            $passwordForm.off('submit').on('submit', function(e){
                ...
                fetch($(form).attr('action') || window.location.href, {
                    ...
                    console.log('[Password Form] Response status:', resp.status);
                    return resp.text().then(function(text) {
                        console.log('[Password Form] Raw response (first 500 chars):', text.substring(0, 500));
                        try {
                            return JSON.parse(text);
                        } catch(err) {
                            console.error('[Password Form] JSON parse error:', err);
                            return { success: false, message: 'Invalid JSON response from server.' };
                        }
                    });
                }).then(function(data){
                    ...
                    if (data && data.success) {
                        if (typeof Swal !== 'undefined') {
                            Swal.fire({ icon: 'success', toast: true, position: 'top-end', showConfirmButton: false, timer: 1800, title: data.message || 'Password updated' });
                        } else {
                        ...
                        if (typeof Swal !== 'undefined') {
                            Swal.fire({ icon: 'error', toast: true, position: 'top-end', showConfirmButton: false, timer: 3000, title: data && data.message ? data.message : 'Error updating password' });
                        } else {
                            alert(data && data.message ? data.message : 'Error updating password');
                        }
                    }
                }).catch(function(err){
                    console.error('Password AJAX failed', err);
                    if (typeof Swal !== 'undefined') {
                        Swal.fire({ icon: 'error', title: 'Network error', text: 'Unable to update password. Please try again.' });
                    } else {
    ...
    } catch(e) { console.warn('Profile handlers not attached', e); }
    ...
    try {
        ...
        if (sampleToggle) console.debug('[initPage] toggleBtn data-link=', sampleToggle.getAttribute('data-link'));
        if (sampleDelete) console.debug('[initPage] deleteBtn data-link=', sampleDelete.getAttribute('data-link'));
    } catch(e) { /* ignore */ }
}
    ...
    try {
        ...
        $links.each(function(){
            ...
            try {
                ...
                if (currentPath === lp) {
                    bestMatch = this; bestLen = lp.length; return false; // exact match -> stop
                }
                if (currentPath.indexOf(lp) === 0 && lp.length > bestLen) {
                    bestMatch = this; bestLen = lp.length;
                }
            } catch(e) {
    ...
    fetch(fetchUrl, { credentials: 'same-origin', headers: { 'X-Requested-With': 'XMLHttpRequest' } })
        .then(function(resp){
            ...
            if (resp.status === 401 || resp.status === 403) {
                console.warn('[loadPage] server returned', resp.status, '— reloading top-level to trigger login redirect');
        ...
        .then(function(html){
            try {
                ...
                try {
                    var loginForm = doc.querySelector('form[action*="/Users/login"], form[action*="/users/login"]');
                    var authBtn = doc.querySelector('.auth-form-btn');
                    var h4 = doc.querySelector('h4');
                    var loginHeading = (h4 && h4.textContent && h4.textContent.indexOf('Welcome to GENTA') !== -1);
                    ...
                    if (loginForm || authBtn || loginHeading) {
                        ...
                        console.warn('[loadPage] detected login HTML from AJAX response', {
                            url: fetchUrl,
                            loginForm: !!loginForm,
                            authBtn: !!authBtn,
                ...
                if (newContent && curContent) {
                    try {
                        ...
                        curContent.style.transition = curContent.style.transition || 'opacity 220ms ease';
                    ...
                    setTimeout(function(){
                            try {
                                ...
                                var curHeight = 0;
                                try { curHeight = Math.ceil(curContent.getBoundingClientRect().height); } catch(e) { curHeight = 0; }
                                if (curHeight > 0) {
                                ...
                                try {
                                    var sampleLink = temp.querySelector('[data-link]');
                                    if (sampleLink) {
                                        console.debug('[loadPage] sample data-link in temp:', sampleLink.getAttribute('data-link'));
                                    }
                                } catch(e) { /* ignore */ }
                                ...
                                scripts.forEach(function(s){
                                    scriptList.push({ src: s.src || null, text: s.textContent || '' });
                                    if (s.parentNode) s.parentNode.removeChild(s);
                                });

                                // Fast replace children with minimal DOM thrash
                                while (curContent.firstChild) curContent.removeChild(curContent.firstChild);
                                Array.prototype.slice.call(temp.childNodes).forEach(function(n){ curContent.appendChild(n); });
                                ...
                                scriptList.forEach(function(item){
                                    try {
                                        ...
                                        setTimeout(function(){ if (ns.parentNode) ns.parentNode.removeChild(ns); }, 0);
                                    } catch(e) { /* ignore script injection errors */ }
                                });

                                // Initialize page widgets while content is already in DOM but hidden
                                try { initPage(); } catch(e) { /* ignore */ }
                                        ...
                                        if ($ && $.fn && $.fn.DataTable) {
                                            $('.defaultDataTable').each(function(){
                                                try {
                                                    ...
                                                    if (tbl) {
                                                        try { tbl.columns().adjust(); } catch(e) {}
                                                        try { if (tbl.responsive) tbl.responsive.recalc(); } catch(e) {}
                                                        try { tbl.draw(false); } catch(e) {}
                                                    }
                ...
                if (newCsrf) {
                    var curCsrf = document.querySelector('meta[name="csrfToken"]');
                    if (curCsrf) curCsrf.setAttribute('content', newCsrf.getAttribute('content'));
                ...
                if (pushState && window.history && history.pushState) {
                    ...
                    try { setTimeout(updateActiveNav, 10); } catch(e) { /* ignore */ }
                }
            } catch (e) {
...
$(document).ready(function() {
    ...
    if (csrfToken) {
        ...
        console.warn('CSRF Token meta tag not found! AJAX requests will fail with 403.');
    }
    // ============================================================
    ...
    const WalkthroughSystem = {
        ...
        injectStyles() {
            ...
            style.innerHTML = `
                ...
                .walkthrough-highlight {
                    ...
                    box-shadow: 0 0 0 5px #b66dff, 
                                0 0 30px rgba(182, 109, 255, 0.8),
                                0 0 60px rgba(182, 109, 255, 0.4) !important;
                ...
                @keyframes pulse-highlight {
                    0%, 100% { 
                        box-shadow: 0 0 0 5px #b66dff, 
                                    0 0 30px rgba(182, 109, 255, 0.8),
                                    0 0 60px rgba(182, 109, 255, 0.4);
                    }
                    50% { 
                        box-shadow: 0 0 0 8px #b66dff, 
                                    0 0 50px rgba(182, 109, 255, 1),
                                    0 0 100px rgba(182, 109, 255, 0.6);
                    }
                }
                ...
                .walkthrough-progress-fill {
                    height: 100%;
                    background: linear-gradient(90deg, #b66dff, #9055e0);
                    border-radius: 4px;
                ...
                .walkthrough-popup .swal2-confirm {
                    background: linear-gradient(135deg, #b66dff, #9055e0) !important;
                ...
                .walkthrough-popup .swal2-confirm:hover {
                    ...
                    box-shadow: 0 4px 12px rgba(182, 109, 255, 0.4);
                }
        ...
        detectCurrentPage() {
            ...
            if (path.includes('/questions') || path.includes('/quiz')) return 'questions';
        ...
        disableScroll() {
            ...
            window.addEventListener('scroll', this.boundPreventScroll, { passive: false });
            window.addEventListener('wheel', this.boundPreventScroll, { passive: false });
            window.addEventListener('touchmove', this.boundPreventScroll, { passive: false });
        },
        ...
        enableScroll() {
            ...
            if (this.originalOverflow) {
                document.documentElement.style.overflow = this.originalOverflow.html;
            ...
            if (this.boundPreventScroll) {
                ...
                window.removeEventListener('touchmove', this.boundPreventScroll);
            }
        },
        ...
        showStep() {
            ...
            if (step.target) {
                ...
                if ($target.length === 0) {
                    console.log('Skipping step, target not found:', step.target);
                    this.currentStep++;
                    this.showStep();
                    return;
                }
            ...
            const html = `
                ${step.icon ? `<div class="walkthrough-icon">${step.icon}</div>` : ''}
            ...
            const swalConfig = {
                ...
                position: step.target ? this.calculatePosition($target, step.position) : 'center',
                didOpen: () => {
        ...
        calculatePosition($target, preferredPosition) {
            ...
            if (preferredPosition === 'right' && rect.right + popupWidth + spacing < viewportWidth) {
                return 'right';
            } else if (preferredPosition === 'left' && rect.left - popupWidth - spacing > 0) {
                return 'left';
            } else if (preferredPosition === 'bottom' && rect.bottom + popupHeight + spacing < viewportHeight) {
                return 'bottom';
            } else if (preferredPosition === 'top' && rect.top - popupHeight - spacing > 0) {
                return 'top';
            }
        ...
        positionPopup($target, position) {
            setTimeout(() => {
                ...
                this.createArrow(popup, rect, arrowPosition, top, left, popupRect);
            }, 50);
        },
        
        createArrow(popup, targetRect, arrowPosition, popupTop, popupLeft, popupRect) {
            ...
            switch (arrowPosition) {
                case 'arrow-left': // Arrow points left (popup is on the right of target)
                    ...
                    if (Math.abs(targetCenterY - (popupTop + popupRect.height / 2)) > 50) {
                        arrowTop = Math.max(popupTop + 20, Math.min(popupTop + popupRect.height - 50, targetCenterY - 18));
                    }
                    break;
                ...
                case 'arrow-top': // Arrow points up (popup is below target)
                    ...
                    if (Math.abs(targetCenterX - (popupLeft + popupRect.width / 2)) > 50) {
                        arrowLeft = Math.max(popupLeft + 20, Math.min(popupLeft + popupRect.width - 50, targetCenterX - 18));
                    }
                    break;
        ...
        complete() {
            ...
            // Mark as completed
            if (!this.isHelpMode && typeof window.walkthrough_shown !== 'undefined' && !window.walkthrough_shown) {
                $.post('/users/set-walkthrough-shown').done(function(data) {
            ...
            setTimeout(() => {
                Swal.fire({
                    title: '<span style="color: #b66dff;">🎉 Tour Complete!</span>',
    ...
    var walkthroughShown = (typeof window.walkthrough_shown !== 'undefined') ? window.walkthrough_shown : false;
    ...
    if (!document.getElementById('walkthrough-highlight-style')) {
        ...
        style.innerHTML = `
            .walkthrough-highlight {
                box-shadow: 0 0 0 4px #b66dff, 0 2px 16px rgba(0,0,0,0.15);
    ...
    var walkthroughShown = (typeof window.walkthrough_shown !== 'undefined') ? window.walkthrough_shown : ((typeof $.cookie === 'function') ? $.cookie('walkthrough_shown') : (document.cookie.indexOf('walkthrough_shown=1') !== -1));
    if (typeof Swal !== 'undefined' && !walkthroughShown) {
        setTimeout(function() { showAnchoredWalkthrough(false); }, 600); // Delay to ensure page is ready
    }
    ...
    $(document).on('click', '#help-walkthrough-btn', function(e) {
        ...
        if (typeof $.removeCookie === 'function') {
            ...
            document.cookie = 'walkthrough_shown=; path=/; expires=Thu, 01 Jan 1970 00:00:00 UTC;';
        }
    ...
    $(document).on('click', '.deleteQuestionBtn', function(e) {
        ...
        Swal.fire({
            ...
            var csrf = $('meta[name="csrfToken"]').attr('content') || '';
            var url = '/teacher/dashboard/delete-question/' + questionId;
            ...
            .then(function(data) {
                if (data.success) {
                    ...
                    Swal.fire({
                        icon: 'success',
                        title: 'Deleted!',
                        text: data.message || 'Question deleted successfully',
                        timer: 1500,
    ...
    $(document).on('click', '.deleteAssessmentsBtn', function(e) {
        ...
        if (!studentHash || !subjectHash) {
            console.error('Missing student or subject id for deleteAssessments');
            return;
        }
        ...
        Swal.fire({
            ...
            var csrf = $('meta[name="csrfToken"]').attr('content') || '';
            var url = '/teacher/dashboard/delete-assessments/' + studentHash + '/' + subjectHash;
            ...
            .then(function(data) {
                if (data.success) {
                    ...
                    Swal.fire({ icon: 'success', title: 'Deleted', text: data.message || 'Assessments deleted', timer: 1500, showConfirmButton: false });
                } else {
                    Swal.fire({ icon: 'error', title: 'Error', text: data.message || 'Failed to delete assessments' });
                }
            }).catch(function(err) {
    ...
    $(document).on('click', 'a.doc-link', function(e) {
        ...
        var url = '/teacher/dashboard/check-document/' + studentHash + '/' + type;
        fetch(url, {
            method: 'GET',
            credentials: 'same-origin',
            headers: { 'X-Requested-With': 'XMLHttpRequest' }
            ...
            if (data && data.exists) {
                ...
                if (typeof Swal !== 'undefined') {
                    Swal.fire({ icon: 'info', title: 'Not available', text: 'There is no available ' + (type === 'tailored' ? 'tailored module' : 'analysis document') + ' to download for this student.' });
                } else {
                    alert('There is no available ' + (type === 'tailored' ? 'tailored module' : 'analysis document') + ' to download for this student.');
                }
            }
        }).catch(function(err){
    ...
    $(document).on('click', '.toggleQuestionStatusBtn', function(e) {
        ...
        Swal.fire({
            ...
            var csrf = $('meta[name="csrfToken"]').attr('content') || '';
            var url = '/teacher/dashboard/toggle-question-status/' + questionId;
            ...
            .then(function(data) {
                if (data.success) {
                    ...
                    if (data.newStatus === 1) {
                        ...
                        $badge.removeClass('bg-success').addClass('bg-secondary').text('Suspended');
                    }
    ...
    $(document).on('click', '#sidebar .nav-item:not(.nav-profile) a.nav-link', function(e) {
        var $a = $(this);
        var href = $a.attr('href');
        ...
        try {
            ...
            $('#sidebar .nav-item:not(.nav-profile) a.nav-link').removeClass('active').closest('.nav-item').removeClass('active');
            $a.addClass('active').closest('.nav-item').addClass('active');
            loadPage(href);
        } catch (err) {
            console.warn('Sidebar AJAX navigation failed, falling back to full load', err);
            window.location.href = href;
        }
    });
    
    // Profile card click handler (top of sidebar)
    $(document).on('click', '#sidebar .nav-item.nav-profile a.nav-link', function(e) {
        var $a = $(this);
        var href = $a.attr('href');
        if (!href || $a.is('[data-no-ajax]')) return;
    ...
    $(document).on('click', 'a.nav-link, a.navbar-brand, a.menu-title, a:not([target="_blank"])', function(e) {
        var $a = $(this);
        var href = $a.attr('href');
        if (!href) return;
    ...
    $(document).on('click', 'a[href*="createEditQuestion"]:not(.btn-edit-question):not(.btn-add-question)', function(e) {
        var href = $(this).attr('href');
        if (!href) return;
        e.preventDefault();
        e.stopImmediatePropagation();
    ...
    $(document).on('click.logout', 'a[href*="/Users/logout"], a.nav-link[href*="/Users/logout"], a[href$="/Users/logout"], a[href$="/users/logout"]', function(e) {
        e.preventDefault();
        var logoutUrl = $(this).attr('href');
        console.log('[SweetAlert2 Logout] Clicked logout link:', logoutUrl, 'Swal:', typeof Swal);
        if (typeof Swal !== 'undefined') {
            Swal.fire({
                title: 'Are you sure?',
    ...
    window.openQuestionFormModal = function(url, title) {
        ...
        questionModalEl.find('.modal-body').html('<div class="text-center"><div class="spinner-border text-primary" role="status"><span class="visually-hidden">Loading...</span></div></div>');
        ...
        $.ajax({
            ...
            questionModalEl.find('form').off('submit').on('submit', function(e) {
                e.preventDefault();
                var $form = $(this);
                    ...
                    if (response.success) {
                        ...
                        if (response.errors) {
                            $.each(response.errors, function(field, msgs) {
                                ...
                                var $feedback = $('<div class="invalid-feedback d-block"></div>').text(msgs.join(', '));
            ...
            questionModalEl.find('.modal-body').html('<div class="alert alert-danger">Failed to load form. Please try again.</div>');
        });
    };
    ...
    $(document).on('click', '.btn-add-question', function(e) {
        ...
        var href = baseUrl + '/teacher/dashboard/createEditQuestion';
    ...
    window.openStudentFormModal = function(url, title) {
        ...
        $modal.find('.modal-body').html('<div class="text-center"><div class="spinner-border text-primary" role="status"><span class="visually-hidden">Loading...</span></div></div>');
        ...
        if (typeof bootstrap !== 'undefined' && bootstrap.Modal) {
            try {
                ...
                $modal.addClass('show').css('display', 'block');
                if ($('.modal-backdrop').length === 0) $('<div class="modal-backdrop fade show"></div>').appendTo(document.body);
            }
        } else {
            $modal.addClass('show').css('display', 'block');
            if ($('.modal-backdrop').length === 0) $('<div class="modal-backdrop fade show"></div>').appendTo(document.body);
        ...
        if (!url) return;
        $.ajax({ url: url, method: 'GET', headers: { 'X-Requested-With': 'XMLHttpRequest' }, cache: false })
            .done(function(html) {
                try {
                ...
                $modal.find('form').off('submit').on('submit', function(e){
                    ...
                    var csrf = $('meta[name=csrfToken]').attr('content') || '';
                    $.ajax({ url: $form.attr('action'), method: method, data: $form.serialize(), dataType: 'json', headers: { 'X-CSRF-Token': csrf } })
                        .done(function(res){
                                if (res && res.success) {
                                    ...
                                    try {
                                        ...
                                        function generateActionButtonsHtml(sobj) {
                                            var viewUrl = '/teacher/dashboard/student/' + sobj.id;
                                            var editUrl = '/teacher/dashboard/editStudent/' + sobj.id;
                                            var deleteUrl = '/teacher/dashboard/deleteStudent/' + sobj.id;
                                            var escapedName = $('<div>').text(sobj.name || '').html();
                                            var html = '';
                                            html += '<a class="btn btn-sm btn-outline-secondary btn-view-student" href="' + viewUrl + '" title="View"><i class="mdi mdi-eye-outline"></i></a> ';
                                            html += '<a class="btn btn-sm btn-outline-primary btn-edit-student" href="#" data-href="' + editUrl + '" title="Edit" data-no-ajax="true"><i class="mdi mdi-pencil"></i></a> ';
                                        ...
                                        if (dt && s && s.id) {
                                            ...
                                            var rowSelector = $(dt.rows().nodes()).filter(function(){
                                            ...
                                            if (rowSelector && rowSelector.length) {
                                                ...
                                                $(newRow).attr('data-id', String(s.id || ''));
                                                try { $(newRow).find('td').eq(3).addClass('text-center').css('white-space', 'nowrap'); } catch(e){}
                                            }
                                        }
                                    } catch (err) {
                                        console.warn('Student table update failed', err);
                                    }

                                    // Hide modal
                                    try { var inst = $modal.data('bs.instance'); if (inst && typeof inst.hide === 'function') inst.hide(); else { $modal.removeClass('show').css('display','none'); $('.modal-backdrop').remove(); } } catch(e){}
                                    if (window.Swal && typeof Swal.fire === 'function') {
                                        Swal.fire({ icon: 'success', title: 'Success', text: res.message || 'Saved', timer: 1500, showConfirmButton: false });
                                    }
                                } else {
                                if (res && res.errors) {
                                    $.each(res.errors, function(field, errs){
                                        ...
                                        $input.addClass('is-invalid');
                                        $modal.find('.invalid-feedback[data-field="' + field + '"]').removeClass('d-none').text((errs && errs.join) ? errs.join(', ') : errs);
                                    });
                                } else {
                                    var msg = (res && res.message) ? res.message : 'Please check the form for errors.';
                                    if (window.Swal && typeof Swal.fire === 'function') Swal.fire({icon:'error', title:'Error', text: msg}); else alert(msg);
                                }
                            }
                        }).fail(function(jqXHR){
                            console.error('Student form submit failed', jqXHR.status, jqXHR.responseText);
                            if (window.Swal && typeof Swal.fire === 'function') Swal.fire({icon:'error', title:'Error', text:'Server error'}); else alert('Server error');
                        });
                });
            }).fail(function(jqXHR){
                ...
                if (resp && resp.length > 50) {
                    ...
                    $modal.find('.modal-body').html('<div class="text-danger text-center">Failed to load form. (' + jqXHR.status + ')</div>');
                }
            });
    };

    // Early delegated handler so clicks are handled even before per-page scripts run
    $(document).on('click', '.btn-add-student, .btn-edit-student', function(e){
        e.preventDefault();
        e.stopImmediatePropagation();
        var raw = $(this).data('href') || $(this).attr('href') || '';
        var href = (raw && raw !== '#' && raw !== 'javascript:void(0)') ? raw : $(this).data('href');
        var title = $(this).hasClass('btn-add-student') ? 'Add Student' : 'Edit Student';
    ...
    $(document).on('click', '#studentModal [data-bs-dismiss="modal"], #studentModal .btn-close', function(e){
        e.preventDefault();
        var $m = $('#studentModal');
        try {
    ...
    $(document).on('click', '.modal-backdrop', function(e){
        ...
        if ($m.length && $m.hasClass('show')) {
            try { var inst = $m.data('bs.instance'); if (inst && typeof inst.hide === 'function') { inst.hide(); return; } } catch(e){}
            $m.removeClass('show').css('display','none');
            $('.modal-backdrop').remove();
        }
    });
        ...
        function doDelete() {
            $.post(url, {_csrfToken: $('meta[name=csrfToken]').attr('content')}).done(function(res){
                if (res && res.success) {
                    ...
                    try {
                        if ($.fn && $.fn.DataTable && $.fn.DataTable.isDataTable('.defaultDataTable')) {
                    ...
                    } catch (err) { console.warn('Delete: could not update DataTable', err); }

                    if (window.Swal && typeof Swal.fire === 'function') {
                        Swal.fire({icon:'success', title:'Deleted', text: res.message || 'Deleted'});
                    } else { alert(res.message || 'Deleted'); }
                } else {
                    var errMsg = (res && res.message) ? res.message : 'Could not delete.';
                    if (window.Swal && typeof Swal.fire === 'function') {
                        Swal.fire({icon:'error', title:'Error', text: errMsg});
                    } else { alert(errMsg); }
                }
            }).fail(function(jqXHR){
                console.error('Student delete failed', jqXHR.status, jqXHR.responseText);
                if (window.Swal && typeof Swal.fire === 'function') {
    ...
    $(document).on('click', 'a[href*="createEditQuestion"]:not(.btn-add-question):not(.btn-edit-question)', function(e) {
    ...
    $(document).on('click', '#questionModal [data-bs-dismiss="modal"], #questionModal .btn-close', function(e) {
        e.preventDefault();
        window.hideQuestionModal();
    });
    ...
    $(document).on('keydown', function(e) {
        ...
        if (isShown && (e.key === 'Escape' || e.key === 'Esc' || e.keyCode === 27)) {
            window.hideQuestionModal();
        }
    });
});